...
content_id: 8442082e458508763cf8f1caf70bd154f47801cb5e2dc928032447ccd828fc01
---
...
Deno.test("Principle: User adds liked dish, preference recorded, influences recommendations", async () => {
  ...
  try {
    console.log(`Action: adds to liked dishes.`);
    ...
    console.log(`Query: Get liked dishes for.`);
    ...
    console.log("Successfully added User's preference.");
  } finally {
    await client.close();
  }
});
...
Deno.test("Action: addLikedDish - Success scenarios", async (t) => {
  ...
  try {
    await t.step("Add liked dish for a new user", async () => {
      console.log(`Action: adds to liked dishes (new user).`);
      const result = await concept.addLikedDish({
        user: userAlice,
        dish: dishPizza,
      });
    ...
    await t.step("Add liked dish for an existing user", async () => {
      console.log(`Action: adds to liked dishes (existing user).`);
      const result = await concept.addLikedDish({
        user: userAlice,
        dish: dishSushi,
      });
    ...
    await t.step("Add an already liked dish (idempotency)", async () => {
      console.log(`Action: adds same liked dish again.`);
      const result = await concept.addLikedDish({
        user: userAlice,
        dish: dishPizza,
      });
...
Deno.test("Action: addLikedDish - Requirement enforcement (failure scenarios)", async (t) => {
  ...
  try {
    await t.step(
      "Fail to add liked dish if already in dislikedDishes",
      async () => {
        console.log(`Action: add to dislikes.`);
        ...
        console.log(`Action: add same item to likes.`);
        const result = await concept.addLikedDish({
          user: userBob,
          dish: dishBurger,
        });
        assertEquals(
          "error" in result,
          true,
          "Expected addLikedDish to fail if dish is disliked.",
        );
        assertEquals(
          (result as { error: string }).error,
          `Dish ${dishBurger} is in disliked dishes for user ${userBob}`,
        );
        ...
        assertEquals(
          (liked as { dish: ID }[]).length,
          0,
          `Expected ${dishBurger} not to be in liked dishes.`,
        );
      },
    );
    ...
    await t.step(
      ...
      async () => {
        ...
        console.log(`Action: add to likes.`);
        const result = await concept.addLikedDish({
          user: userAlice,
          dish: dishSalad,
        });
...
Deno.test("Action: addDislikedDish - Requirement enforcement (failure scenarios)", async (t) => {
  ...
  try {
    await t.step(
      "Fail to add disliked dish if already in likedDishes",
      async () => {
        ...
        assertEquals(
          "error" in result,
          true,
          "Expected addDislikedDish to fail if dish is liked.",
        );
        assertEquals(
          (result as { error: string }).error,
          `Dish ${dishPizza} is in liked dishes for user ${userAlice}`,
        );
        ...
        assertEquals(
          (disliked as { dish: ID }[]).length,
          0,
          `Expected ${dishPizza} not to be in disliked dishes.`,
        );
      },
    );
...
Deno.test("Queries: _getLikedDishes and _getDislikedDishes", async (t) => {
  ...
  try {
    ...
    await t.step(
      "Get liked dishes for a user with no preferences",
      async () => {
        console.log(
          `Query: Get liked dishes for ${userCharlie} (no preferences).`,
        );
    ...
    await t.step(
      "Get disliked dishes for a user with no preferences",
      async () => {
        console.log(
          `Query: Get disliked dishes for ${userCharlie} (no preferences).`,
        );
    ...
    await t.step(
      "Get liked dishes for a user that only exists in liked, not disliked",
      async () => {
