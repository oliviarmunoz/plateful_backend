---
timestamp: 'Thu Oct 16 2025 23:38:01 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251016_233801.40713858.md]]'
content_id: cb3aea98a6bb14d5bc163a23f3f65c0bed9c38eb644cfb062e1bbc05db399ca9
---
...
Deno.test("Principle: User preferences are recorded and updated, influencing recommendations (via internal state)", async () => {
  ...
  try {
    ...
    assertEquals("error" in likedDishesQueryResult[0], false, "Query for liked dishes should not return an error.");
    ...
    let dislikedDishesQueryResult = await concept._getDislikedDishes({ user: userA });
    assertEquals("error" in dislikedDishesQueryResult[0], false, "Query for disliked dishes should not return an error.");
...
Deno.test("Action: addLikedDish - new user, existing user, existing disliked dish", async () => {
  ...
  try {
    ...
    assertEquals("error" in likedQueryResult[0], false, "Query for liked dishes should not return an error.");
    ...
    let dislikedQueryResultB = await concept._getDislikedDishes({ user: userB });
    assertEquals("error" in dislikedQueryResultB[0], false, "Query for disliked dishes should not return an error.");
    ...
    assertEquals("error" in likedQueryResultB[0], false, "Query for liked dishes should not return an error.");
...
Deno.test("Action: removeLikedDish - existing, non-existent dish, non-existent user", async () => {
  ...
  try {
    ...
    let likedQueryResult = await concept._getLikedDishes({ user: userA });
    assertEquals("error" in likedQueryResult[0], false, "Setup query for liked dishes should not return an error.");
    let liked = likedQueryResult as { dishes: ID }[];
    assertEquals(
      liked.map((d) => d.dishes).sort(),
      [dish1, dish2].sort(),
    ...
    likedQueryResult = await concept._getLikedDishes({ user: userA });
    assertEquals("error" in likedQueryResult[0], false, "Query after removal should not return an error.");
    liked = likedQueryResult as { dishes: ID }[];
    assertEquals(
      liked.map((d) => d.dishes),
      [dish2],
...
Deno.test("Action: addDislikedDish - new user, existing user, existing liked dish", async () => {
  ...
  try {
    ...
    assertEquals("error" in dislikedQueryResult[0], false, "Query for disliked dishes should not return an error.");
...
Deno.test("Action: removeDislikedDish - existing, non-existent dish, non-existent user", async () => {
  ...
  try {
    ...
    let dislikedQueryResult = await concept._getDislikedDishes({ user: userA });
    assertEquals("error" in dislikedQueryResult[0], false, "Setup query for disliked dishes should not return an error.");
    let disliked = dislikedQueryResult as { dishes: ID }[];
    assertEquals(
      disliked.map((d) => d.dishes).sort(),
      [dish1, dish2].sort(),
    ...
    dislikedQueryResult = await concept._getDislikedDishes({ user: userA });
    assertEquals("error" in dislikedQueryResult[0], false, "Query after removal should not return an error.");
    disliked = dislikedQueryResult as { dishes: ID }[];
    assertEquals(
      disliked.map((d) => d.dishes),
      [dish2],
...
Deno.test("Query: _getLikedDishes - existing user with dishes, existing user no dishes, non-existent user", async () => {
  ...
  try {
    ...
    likedQueryResult = await concept._getLikedDishes({ user: userA });
    // For an empty array, it won't have result[0], so we check it directly.
    assertEquals(
      likedQueryResult,
      [],
...
Deno.test("Query: _getDislikedDishes - existing user with dishes, existing user no dishes, non-existent user", async () => {
  ...
  try {
    ...
    dislikedQueryResult = await concept._getDislikedDishes({ user: nonExistentUser });
