---
timestamp: 'Thu Oct 16 2025 23:33:53 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251016_233353.a6a65a0d.md]]'
content_id: 9e7aca2793378ebb1eeb35b637806fe5b8c000a6344116ee9ee5063ea7bd584b
---
...
// freshID is not directly used for User _id in upsert as User is a generic ID,
// but included for completeness if internal entities needed IDs.
...
export default class UserTastePreferencesConcept {
  ...
  constructor(private readonly db: Db) {
    ...
    // Optionally, ensure indexes for performance, e.g., on _id if not implicitly indexed.
  ...
  async addLikedDish({ user, dish }: { user: User; dish: Dish }): Promise<Empty | { error: string }> {
    const result = await this.users.findOneAndUpdate(
      { _id: user },
      {
        ...
        // Initialize likedDishes and dislikedDishes arrays if a new document is inserted
    ...
    if (result.ok) {
      ...
      // In case of a database write error
      return { error: "Failed to add liked dish due to an internal database error." };
    }
  }
  ...
  async removeLikedDish({ user, dish }: { user: User; dish: Dish }): Promise<Empty | { error: string }> {
    ...
    if (!existingUser.likedDishes.includes(dish)) {
      return { error: `Dish '${dish}' is not in the liked dishes for user '${user}'.` };
    }
    ...
    if (result.acknowledged) {
      ...
      return { error: "Failed to remove liked dish due to an internal database error." };
    }
  }
  ...
  async addDislikedDish({ user, dish }: { user: User; dish: Dish }): Promise<Empty | { error: string }> {
    ...
    if (result.ok) {
      ...
      // In case of a database write error
      return { error: "Failed to add disliked dish due to an internal database error." };
    }
  }
  ...
  async removeDislikedDish({ user, dish }: { user: User; dish: Dish }): Promise<Empty | { error: string }> {
    ...
    if (!existingUser.dislikedDishes.includes(dish)) {
      return { error: `Dish '${dish}' is not in the disliked dishes for user '${user}'.` };
    }
    ...
    if (result.acknowledged) {
      ...
      return { error: "Failed to remove disliked dish due to an internal database error." };
    }
  }
  ...
  async _getLikedDishes({ user }: { user: User }): Promise<{ dishes: Dish }[] | [{ error: string }]> {
    ...
    const existingUser = await this.users.findOne({ _id: user }, { projection: { likedDishes: 1 } });
  ...
  async _getDislikedDishes({ user }: { user: User }): Promise<{ dishes: Dish }[] | [{ error: string }]> {
    // Precondition check: user must exist
    const existingUser = await this.users.findOne({ _id: user }, { projection: { dislikedDishes: 1 } });
